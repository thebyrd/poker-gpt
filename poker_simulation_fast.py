...
class FastMonteCarloStrategy:
    ...
    def best_hand_from_ranks_and_suits(ranks, suits):
        ...
        if np.any(suit_counts >= 5):
            ...
            if FastMonteCarloStrategy.is_straight(flush_ranks):
        ...
        # Check for straight
        if FastMonteCarloStrategy.is_straight(ranks):
    ...
    def monte_carlo_simulation_vectorized(player_hand, community_cards, num_opponents=1, n_simulations=10000):
        deck = FastMonteCarloStrategy.generate_deck()
        ...
        player_ranks = np.array([FastMonteCarloStrategy.best_hand_from_ranks_and_suits(FastMonteCarloStrategy.extract_ranks(np.concatenate([player_hands[i], final_community_cards[i]])),
                                                            FastMonteCarloStrategy.extract_suits(np.concatenate([player_hands[i], final_community_cards[i]])))[0]
                                for i in range(n_simulations)])

        opponent_ranks = np.array([
            np.max([FastMonteCarloStrategy.best_hand_from_ranks_and_suits(FastMonteCarloStrategy.extract_ranks(np.concatenate([opponent_hands[i, j], final_community_cards[i]])),
                                                FastMonteCarloStrategy.extract_suits(np.concatenate([opponent_hands[i, j], final_community_cards[i]])))[0]
    ...
    def simulate(self, player, game_state):
        ...
        win_probability, _ = FastMonteCarloStrategy.monte_carlo_simulation_vectorized(hole_cards, community_cards, num_opponents=len(game_state['players']) - 1, n_simulations=self.simulations)
...
class KellyCriterionStrategy:
    def __init__(self, simulations=10000):
        ...
        self.monte_carlo = FastMonteCarloStrategy(simulations=simulations)
...
if __name__ == "__main__":
    ...
    if args.test:
        ...
        player1 = Player("Alice", KellyCriterionStrategy(simulations=10000)) # Alice should win b/c she has more simulations
